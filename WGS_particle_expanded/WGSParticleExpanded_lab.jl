...
h_f_val = h_f_func(T_val, C_i_val, D_cat_val, D_rct_val, F_0)
...
# WGSParticleExpanded
STEP_DE = [C_p_c_i[i] * C_c_i[i] * Dt(T_c(t, r)) for i in 1:5]
DE5 = [(1 - θ) * ρ_cat * C_p_cat * Dt(T_c(t, r)) + θ * sum(STEP_DE) ~ (((2 * λ_cat) / r) * Dr(T_c(t, r)) + λ_cat * Drr(T_c(t, r))) + θ * (D_i_m[i] * Dr(C_c_i[i]) * C_p_c_i[i] * Dr(T_c(t, r))) for i in 1:5]
...
BCS4 = [h_f * (T_c(t, rad_cat) -  T[i]) + sum(H_i .* k_c_i .* (C_c_i_rad - C_i)) ~ (- λ_cat) * Dr(T_c(t, rad_cat)) - sum(STEP_BCS) for i in 1:5]
...
vars_dep = [D_1_m, D_2_m, D_3_m, D_4_m, D_5_m, r_1, r_2, r_3, r_4, r_5]
